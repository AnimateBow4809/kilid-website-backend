...
public interface PictureRepo extends JpaRepository<Picture, PictureKey> {
    ...
    @Query("select Picture  from Picture p WHERE p.pictureKey.propertyID=:propertyID ")
    List<Picture> getPicturesById(@Param(value = "propertyID") Long propertyID);
    @Query("select Picture  from Picture p WHERE p.pictureKey.propertyID=:propertyID and p.pictureKey.pictureID=:pictureID")
